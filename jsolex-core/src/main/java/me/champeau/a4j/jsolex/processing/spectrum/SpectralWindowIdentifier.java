...
public final class SpectralWindowIdentifier {
    ...
    public Identification identify(float[][] data,
        ...
        try {
            ...
            var ranked = DeepLineIdentifier.rank(profile, instrument, pixelSize, binnings);
            if (ranked.isEmpty()) {
            ...
            var refined = DeepLineIdentifier.refine(profile, instrument, pixelSize, result.binning(), result.wavelength(), SCALES);
    ...
    private static List<Line> findLines(List<SpectrumAnalyzer.DataPoint> profile,
        ...
        var atlasLines = DeepLineIdentifier.atlasLines(sigmaIndex,
