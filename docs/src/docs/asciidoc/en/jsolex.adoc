...
- `crop_sunspots` takes a single image and generates a list of images, each one cropped around a detected sunspot. For example: `crop_sunspots(continuum())`. It accepts an optional parameter which is the minimal size of the sunspot, in pixels. For example: `crop_sunspots(continuum(), 10)`. The default size is 32 pixels.
