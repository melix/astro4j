...
The dialog lists every scan available on the device, with its acquisition date, spectral line and processing status. Tick the scans you want to process, choose a download folder, then click "Import and process". JSol'Ex downloads the selected `scan.ser` files and opens them: a single scan opens the regular processing window, while several scans are processed as a batch.
