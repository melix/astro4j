...
Python scripts must assign their result to the `result` variable:
...
Python provides control structures that ImageMath lacks:
...
# Exception handling
try:
    result = jsolex.funcs.load("/path/to/file.fits")
except:
