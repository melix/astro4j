...
- `ar_overlay` generates an overlay of the detected active regions. It accepts a single mandatory parameter: the image on which to draw the overlay. A second parameter tells if the region name should be displayed (1=display, 0=do not display) This feature will only work if active region image generation has been selected. For example: `ar_overlay(continuum())`.
