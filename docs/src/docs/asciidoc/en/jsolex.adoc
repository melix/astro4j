...
- `bg_model` creates a background model from an image, which can be used to subtract the background from the original image. It takes the image to process as the first argument, and an optional polynomial order as the second argument. For example: `bg_model(img(0))`. The default order is 2. It is not recommended to use an order higher than 3.
