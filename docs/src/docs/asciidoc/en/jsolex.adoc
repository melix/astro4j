...
JSol'Ex is a solar images processing software for http://www.astrosurf.com/solex/sol-ex-presentation-en.html[Christian Buil's Sol'Ex] or other spectroheliographs such as the SHG 700 or your own builds.
It is capable of processing SER files captured with this instrument in order to produce images of the solar disk. It was inspired by http://valerie.desnoux.free.fr/inti/[INTI from Valérie Desnoux] but offers many unique features.
...
- the observation bandwidth: while it's not mandatory to select that field, doing so will enable automatic colorization of the images and advanced functionalities such as velocity calculations. By clicking on the "..." you have the possibility to add your own lines if they are not listed. The field next to the list lets you type a wavelength in angstroms directly, for a line you do not want to keep in the list: leave it empty to use the line selected in the list. This is the wavelength of the darkest line in your capture window.
- Line detection: how the observed line is determined.
"Manual selection" uses the wavelength you selected above.
"Automatic search on known lines" looks for it among the lines listed above.
"Automatic search on full spectrum", which is the default, identifies it among all the deep lines of the solar spectrum, so it can process a line which is not listed at all.
When the search recognises a line you already have, that line is used, so its colorization and its scripts still apply; otherwise the line is created for this scan only, named after the spectral line catalog when it is known.
The search only answers when the identification is certain: a narrow capture window may not contain enough spectrum to identify a line, in which case the software falls back on the listed lines.
...
- Contrast enhancement method: you can choose between Autostretch (JSol'Ex), CLAHE and CLAHE (multi-scale). Depending on your choice, additional parameters will be available.
...
The CLAHE (multi-scale) method runs CLAHE several times at tile sizes derived automatically from the solar disk diameter and averages the results. This greatly reduces the bright "ring" artifact that classic CLAHE can produce at the limb, while still enhancing local contrast at multiple scales. Only the clipping factor is configurable; tile sizes and histogram levels are chosen automatically.

Next come the transversallium correction parameters, which allow to correct transversal bands which can appear on images, for example because of dust on the slit.
Two methods are available: banding correction, used by default, and stripe removal (destripe), which is more effective on strong or irregular stripes.
Both methods only use the pixels of the solar disk.

Banding correction has the following parameters:
...
Stripe removal has the following parameters:

- Automatic passes: when checked, passes are repeated until the stripes are removed.
- Passes: the number of passes to perform when automatic passes are unchecked.
- Width: the width, in pixels, of the widest stripes to remove. Structures wider than this are kept.

When debug images are enabled, a graph is also generated, showing the correction applied at each position along the slit by each pass, and the total correction.
...
==== Mount Oscillations Correction

This is an experimental feature available since JSol'Ex 5.3.2.
Some mounts exhibit a regular, periodic oscillation during the scan, which shows up as a sinusoidal wave pattern on the solar limb.
While jagged edges correction can reduce this defect, it corrects each line independently, which makes it sensitive to noise and can introduce artifacts.

The mount oscillations correction instead performs a spectral analysis of the limb position to find the dominant oscillation frequencies, then fits a model of the displacement and applies it to the whole image.
The model is a sinusoid when the oscillation is stable, or a smoothed displacement curve when its period or amplitude drifts during the scan.
Because the correction is driven by a global model rather than per-line measurements, it is insensitive to per-line detection noise, and it can also correct lines where the limb cannot be measured reliably, for example near the poles or outside of the disk.

If no significant periodic oscillation is detected, no correction is applied.
The detected amplitude and period are written to the log, which can help diagnosing the mount.
When debug images are enabled, a graph is also generated, showing the measured limb shift, the fitted oscillation model and the residual shift after correction.
Both corrections can be enabled together: in this case the oscillation is removed first, and the jagged edges correction only deals with the remaining random defects.
...
On the average image, the detected spectral line is drawn in red.
When you select a specific wavelength, JSol'Ex checks that the detected line really is the one you asked for: a neighbouring line is sometimes darker, in particular on narrow cropping windows selected around a faint reference line.
When it has to correct itself, the line which is actually used for the reconstruction is drawn in green, and the red one shows what had been detected.
...
When the solar disk is overexposed (saturated), the spectral line can no longer be distinguished reliably, so JSol'Ex cannot detect the distortion polynomial from the scan itself.
In this situation, you can capture one or more *non saturated* reference scans and let JSol'Ex reuse their polynomial.

Check the "Saturated disk mode" box (below "Force polynomial") and select the directory containing your non saturated reference scans.
When processing a file, JSol'Ex looks up the reference scan which is the closest in time (based on the SER file dates) and applies its polynomial.

This mode is especially useful in batch processing: several saturated scans may share the same reference scan, in which case the reference polynomial is computed only once and reused.

If the closest reference scan is far away in time, a warning is issued because the polynomial may no longer be accurate.
If no usable reference scan is found, JSol'Ex falls back to the regular automatic detection.

This mode also changes how the edge of the solar disk is detected: the glow surrounding a saturated disk is bright enough to be mistaken for the disk, so JSol'Ex looks for the edge of the saturated area instead.
As a consequence, the detected disk is slightly smaller than the true photospheric limb, which is unavoidable with a saturated disk.

NOTE: "Saturated disk mode" and "Force polynomial" are mutually exclusive.
...
You can also duplicate an image from the sidebar by right-clicking on its name and choosing "Duplicate", which is useful when you want to keep a clean original alongside an annotated version.

Animations and videos have a "Crop / rotate..." button which opens a window showing a single frame (the current one for a video).
You can rotate it, flip it or crop it, then click "Apply": the operations are applied to every frame and the result appears as a new animation next to the original one.

=== Other spectral lines in the capture window

The captured window often contains known spectral lines other than the one you are studying. For example, when imaging in Calcium H, the H-epsilon line usually falls within the same window.

When this happens, JSol'Ex marks each of these lines with a dotted vertical marker, labelled with the line name, on the spectral profile graph, and automatically reconstructs a dedicated image at that line's position, exactly as if you had added its pixel shift manually.

=== Annotations

Each image has its own set of annotations that you can turn on or off independently from the annotations panel, opened with the ⊕ button in the toolbar.
The panel is a floating popover that you can drag around the screen while you work.

The following annotations are available:

- *Colorize*: applies a color to a monochrome image, which is useful as a backdrop for colored annotations. This option only appears for monochrome images. From its options menu you can pick a spectral profile (e.g. H-alpha, Calcium, Helium) or a custom color; the default profile matches the spectral line of the image. A "Preserve brightness" option keeps the original luminance so colorizing only adds hue without darkening the image. RGB images are left untouched.
- *Orientation grid*: heliographic meridians and parallels drawn on the solar disk, with the rotation axis and cardinal direction markers. The grid style (equatorial or solar coordinates) and the P-angle correction can be tweaked from the grid options menu.
- *Prominence scale*: dashed concentric circles outside the solar limb with distance labels in kilometres. You can choose the number of circles, the spacing between them, and the angle at which the labels are drawn, so that they don't overlap prominences.
- *Active regions*: the NOAA active region numbers placed at their position on the solar disk. From the options menu you can optionally draw a rectangle around each region, and set the P-angle correction for ImageMath images.
- *Observation details*: a block of text usually placed in the top-left corner with the observer name, date, instrument, telescope, camera, etc. The content is fully editable through a template with placeholders (e.g. `%OBSERVER%`, `%DATETIME%`, `%TELESCOPE%`), so you can pick exactly which fields are shown, reorder them, or add your own text. Empty lines (where the value is missing) are skipped automatically.
- *Solar parameters*: a block listing the P, B0 and L0 angles together with the Carrington rotation, placed in the top-right corner.
- *Earth size reference*: a small Earth picture overlaid on the image as a size comparison. The marker is draggable: click and hold to move it where you want.
- *Signature*: free text rendered in the bottom-left corner. You can pick the font (the dropdown previews each typeface in its own style), the size and the weight.
- *Text areas*: an arbitrary number of free-text annotations that you place anywhere on the image by dragging them. Use the "Add text area" button at the bottom of the panel to create one; each has its own text, font, size, weight and color, and can be removed with its trash button.

Each annotation has its own color, with the default being a neutral gray that preserves monochrome images.
A global line thickness control at the bottom of the panel lets you make all drawn lines bolder or thinner.

When editing the signature or the observation details template, the changes preview live on the image.
Click "Apply" to keep them or "Close" to revert.

You can save the current annotation settings (toggles, colors, line thickness, prominence scale options, observation details template, signature) as a preset with the "Save preset" button at the bottom of the panel, and reload them on any other image with "Load preset".
The preset is stored on your computer and persists across JSol'Ex sessions.

== Exporting and importing sessions

After processing one or more files, you often want to compare the results and later share the best image, for example on SpectroSolHub.
Instead of reprocessing the original SER video, you can export the current state of the image viewer to a *session file* and reopen it whenever you need it.

To export, choose "Export session..." from the *File* menu and pick a destination.
The session file (`.jsxs`) stores every generated image, the animations and videos, and their processing parameters.
The file is compressed, but keep in mind that a session containing many images can still be large.

To restore a session, choose "Import session..." from the *File* menu and select a `.jsxs` file.
If the viewer already contains images, JSol'Ex asks whether you want to replace them or merge the imported ones with the current view.
The restored images keep their original parameters, so you can save, annotate or share them exactly as if they had just been processed, without touching the original video.
...
== Importing scans from a SunScan

If you own a SunScan spectroheliograph, JSol'Ex can fetch the scans acquired by the device directly, without having to copy files manually.

Connect your computer to the SunScan Wi-Fi access point, then in the "File" menu choose "Import from SunScan...".

JSol'Ex will try to locate the device automatically on the network. If detection fails, type its address (for example `sunscan.local:8000`) in the "Device address" field and click "Connect".

The dialog lists every scan available on the device, with its acquisition date, spectral line and processing status. Tick the scans you want to process, choose a download folder, then click "Import and process". JSol'Ex downloads the selected `scan.ser` files (renamed `YYYY_MM_DD-HH_mm_ss-scan.ser` based on each scan's UTC capture date) and opens them: a single scan opens the regular processing window, while several scans are processed as a batch.

When scans are imported this way, the "Sunscan" instrument and its setup (camera and pixel size) are selected automatically in the processing parameters, which also enables the vertical flip of the spectrum. You can still change these settings before starting the processing.
...
[[internal-outputs]]
==== Hiding an intermediate result

Sometimes an intermediate result is only needed by the `[outputs]` section itself, and moving it to another section would make the script harder to read.
In that case, prefix its name with `__`: such a variable is computed and can be used by the other expressions, but it is not considered an output, so it is neither displayed nor saved.

[source]
----
[outputs]
# not an output: only used below
...
* **requirements**: Container for declaring image kinds that must be generated for the script to work (see <<#script-requirements,Script Requirements>>)
* **overrides**: Container for processing parameters which the script forces, whatever the user selected (see <<#script-overrides,Overriding processing parameters>>)

[[script-requirements]]
==== Script Requirements

A script may declare image kinds that it needs to perform its job. When a script is executed, any `GeneratedImageKind` listed in the `requirements` block will be added to the list of images to generate, as if the user had explicitly selected them in the image selection panel.

[source]
----
meta {
    title = "Active regions on continuum"
    requirements {
...
Unknown image kinds are silently ignored. Implied kinds are automatically included (for example `ELLERMAN_BOMBS` implies `FLARES`).

[[script-overrides]]
==== Overriding processing parameters

Some scripts only make sense with particular processing parameters: for example a script which performs its own banding removal needs the built-in banding correction to be disabled, and a script which works on the corona needs a wide enough crop.
Such a script can declare an `overrides` block, which forces processing parameters regardless of what the user selected:
...
The parameter names are those of the `config.json` file which JSol'Ex writes next to the generated images, so any processing parameter can be overridden: open that file to find the name of the parameter you are looking for.
The most useful ones are:

[%header,cols="35%,65%"]
|===
|Parameter|Description
|`bandingCorrectionParams.passes`|Number of banding correction passes. Use `0` to disable the correction.
|`bandingCorrectionParams.width`|Band size, in pixels, of the banding correction.
|`bandingCorrectionParams.method`|Banding correction method: `BANDING_CORRECTION` or `DESTRIPE`.
|`bandingCorrectionParams.destripeParams.passes`|Number of stripe removal passes, `-1` to stop automatically.
|`bandingCorrectionParams.destripeParams.bandSize`|Width, in pixels, of the widest stripes removed by the stripe removal.
|`geometryParams.autocropMode`|Cropping: `OFF`, `RADIUS_1_1`, `RADIUS_1_2`, `RADIUS_1_5`, `SOURCE_WIDTH` or `FIXED_WIDTH`.
|`geometryParams.fixedWidth`|Width used when `autocropMode` is `FIXED_WIDTH`.
|===

Numbers are written as is, everything else between quotes, including booleans: `spectrumVFlip = "true"`.
Only single values can be overridden, not lists.
Unknown parameters and invalid values are ignored, with a warning in the logs.
When several scripts override the same parameter with different values, the first one wins.

The overrides are applied before the video is processed, and the `config.json` saved next to the images reflects the values which were actually used.

Overrides can also be edited from the metadata editor, and are supported by Python scripts, using one comment per parameter:
...
|`inputFilesCount`|The number of files which are processed: 1 when processing a single file, the number of files of the batch in a `[[batch]]` section
...
For advanced scripting capabilities, JSol'Ex also supports Python. Please refer to link:python-scripting.html[the Python scripting documentation] for details.
...
For scripts that require additional files (such as Python modules), you can bundle them in a `.zip` archive.
JSol'Ex will automatically extract the contents when downloading from a repository.
...
----
my-script.zip
├── my-script.math       # Main script
├── main.txt             # Optional: specifies main script (required if multiple .math files)
├── utils.py             # Python modules
└── helpers.py
----

**Rules for determining the main script:**

1. If there is only one `.math` file in the zip, it is automatically used as the main script
2. If there are multiple `.math` files, you must include a `main.txt` file containing the name of the main script

The directory structure inside the zip is preserved, so scripts can reference files using relative paths like `python_file("utils.py")`.
...
List zip files in `scripts.txt` alongside regular `.math` files:
...
WARNING: Back-and-forth scanning is strongly discouraged. Most mounts have a preferred scanning direction, for which the scanning speed is more consistent: scanning in both directions may therefore distort half of your images. Only use this option if you are absolutely sure that scans in both directions are perfect.

Some observers scan alternately in one direction, then in the other, in order to avoid having to move the telescope back to its starting point between two scans.
Because a scan performed in the opposite direction produces a mirrored image, one image out of two would have to be flipped by hand.

Check the "Back-and-forth scanning" option, in the "Batch Options" section of the processing parameters, to let JSol'Ex do it for you.
The files of the batch are then sorted by capture time, and every other scan, starting with the second one, is flipped automatically.

NOTE: If the flip is applied to the wrong scans, for example because your first file happens to be a scan in the reverse direction, simply check the "Flip horizontal axis" option in addition: this reverses which scans are flipped.

[[batch-conditional-flip]]
=== Conditional flips

When the image orientation changes during a session, for example after a meridian flip, a single "flip" checkbox cannot describe a batch where some files must be flipped and others not.

The "Conditional horizontal flip" and "Conditional vertical flip" options, in the "Batch Options" section of the processing parameters, solve this.
When you enable one of them, you define a pivot date, in UTC, and choose whether the files captured before the pivot date, or those captured at or after it, must be flipped.
The other files are not flipped.

Instead of typing the pivot date by hand, you can click on "From SER file..." and select the first video captured after the orientation change: its capture date is then used as the pivot date.

NOTE: When a conditional flip is enabled, it replaces the corresponding "Flip horizontal axis" or "Flip vertical axis" checkbox, which is ignored for that axis.

When the saturated disk mode is also enabled, only reference scans captured on the same side of the pivot date as the file being processed are considered: a meridian flip changes the mechanical flexions, so a reference from the other side would give a biased polynomial.
In this case, make sure the pivot corresponds to the first capture after the flip, reference scans included: if a reference scan was captured between the flip and the file you select as the pivot, it would incorrectly count as captured before the flip.

[[batch-add-files]]
=== Adding files to a batch

Once a batch is finished, you can add more files to it without having to process the files which are already done.
The new files are processed with the same parameters as the rest of the batch, then, if your script has a `\[[batch]]` section, the batch outputs are computed again over all the files, old and new.
The results appear in their own timestamped section, so that you can compare them with the previous ones.

To do this, click on the "Add files..." button, at the top of the batch table, and select the files you want to add.

Alternatively, you can click on the "Watch a directory..." button and select a directory.
JSol'Ex then monitors that directory: each new SER file which appears in it is automatically added to the batch.
This is particularly convenient during an observation session: start a batch with the first scans, then let JSol'Ex add the following ones as your capture software writes them.

Files are only added to the batch once the current processing is done, which means that new files detected while the batch is running are put on hold and will be added as soon as it is finished.
Only the files which appear after you start watching are taken into account: the files which are already in the directory are ignored.
In any case, whether you add files by hand or through the watched directory, a file which is already in the batch is never added a second time.
Click on "Stop watching" to stop monitoring the directory.
...
- the sampling factor determines where samples will be picked when computing the distortion model. A sampling factor of 0.5 combined with a tile size of 32 means that a sample will be picked every 16 pixels.
...
The identification uses the same method as the live identification of the embedded server (see <<live-line-identification>>): the line at the centre of the image is found among all the deep lines of the solar spectrum, as seen through the atmosphere, and every other line visible in the image is identified too, including the telluric lines.
The orientation of the image is detected automatically.
...
The lines found in the image are marked in green, and the status bar tells which line the image is centred on and how confident the identification is.

The "Telluric lines" checkbox shows the reference spectrum as seen through the atmosphere, which is what your camera captures: untick it to see the solar spectrum alone.
...
=== Live identification of spectral lines

The embedded server also exposes an API which identifies the spectral lines visible in a spectrum frame, such as the live view of your capture software.
This lets you know which line you are looking at while tuning the grating of your spectroheliograph, without stopping the capture.

A script for SharpCap is provided: choose "SharpCap scripts" in the "Help" menu to open the folder where JSol'Ex copies it (it is also available https://github.com/melix/astro4j/blob/main/scripts/sharpcap/jsolex_spectral_lines.py[on GitHub]).
Once JSol'Ex is running with its embedded server started, run the script from the SharpCap scripting console.
It sends the live frames to JSol'Ex and draws every identified line over the live view, with its name when it is a known line or its wavelength otherwise.
The line the window is centred on is highlighted, and a status line reports the confidence of the identification, which grows as successive frames agree on the same line.
A line is proposed as soon as the frame carries a spectrum: when the window is too narrow for one line to stand out from all the others, the most likely one is still shown, but the confidence reports it as a guess and its line is drawn dashed.
Widen the capture window or average more frames before trusting such an answer.
Stop the script with Ctrl+C to remove the overlay.

A small settings window opens with the script.
It lets you limit the number of labelled lines, which are then spread evenly over the window while the others are only marked by a tick, and set the size of the labels, which follows the zoom of the display.
It also offers a "Find a line" mode: enter the wavelength you are looking for, in Angstroms, and the script only draws that line when it is in the window, along with a ruler graduated in Angstroms on the right of the spectrum; when the line is out of the window, the status line tells how far beyond which edge it is, so that you know which way to turn the grating.

The binning is read from the camera, and the orientation of the spectrum is detected from the curvature of the lines, so it does not matter which way the capture software reads the sensor.
The spectroheliograph and the pixel size of the camera are those of the observation details of JSol'Ex, unless the script overrides them: edit the parameters at the top of the script to set the address of the server, the pixel size or the name of the spectroheliograph.
The identification gets more reliable as the capture window gets taller, since more of the spectrum is compared with the reference spectrum, and as more frames are averaged.
The frame is compared with a reference which includes the absorption lines of the Earth's atmosphere, taken from the NSO/Kitt Peak atlas: around H-alpha the water lines are as deep as the solar ones, and a spectrum observed from the ground cannot be recognised without them.
Such lines are labelled as telluric.

The API can be used from any other software: send a frame with a `POST` request to `/api/spectrum/identify`.
The body is either the raw pixels of the frame, with the `width`, `height` and `format` query parameters, `format` being the colour space of the camera (`MONO8`, `MONO12`, `MONO16`, `RGB24`, `RGB32`, `RGB48`, `RAW8`, `RAW12` or `RAW16`, multi-byte samples being little-endian), or an image file (PNG, TIFF, or FITS with `format=fits`).
Colour frames are averaged to mono, and frames carrying a Bayer mosaic are kept at full resolution with the mosaic evened out.
The optional `pixelSize`, `binning`, `instrument` and `average` query parameters override the observation details and the number of averaged frames.
The response is a JSON document describing the curvature polynomial of the lines, the identified line with its score, the dispersion, and the list of lines found in the window with their position relative to the centre of the frame.
A `DELETE` request on the same address forgets the previous frames, for example when you switch to another camera.
...
- The National Solar Observatory for the atlas of the telluric spectrum used by the live identification of spectral lines: NSO/Kitt Peak FTS data used here were produced by NSF/NOAO
