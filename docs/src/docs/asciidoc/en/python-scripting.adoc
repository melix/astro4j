...
WARNING: **Windows does not support NumPy or other native extensions.** GraalPy on Windows can only install pure Python packages (no C extensions, no source builds). NumPy, PyArrow, and similar libraries requiring native code will not work. If you need these libraries, use Linux or macOS.

On Windows, you can still use GraalPy for pure Python scripting (no `import numpy`). To set this up, skip to <<graalpy-venv-windows>>.

=== Step 1: Install GraalPy

Download and install GraalPy from the https://www.graalvm.org/python/python-developers/[official GraalPy page].

=== Step 2: Install prerequisites (Linux / macOS only)

NumPy may need to be compiled from source under GraalPy, which requires a C compiler and the `patch` utility.

**Linux**

Install a C compiler and patch if not already available:
...
Choose a directory for your virtual environment, for example `graalpy-venv` in your home folder.
...
NOTE: The installation may take several minutes if NumPy needs to be compiled from source.

[[graalpy-venv-windows]]
**Windows** (pure Python only, no NumPy)

[source,cmd]
----
graalpy -m venv C:\Users\<your-username>\graalpy-venv
----

TIP: On Windows, the virtual environment places executables in the `Scripts` folder (not `bin`). To install pure Python packages, use the `python.exe` from inside the venv with the `--no-cache-dir` flag: `C:\Users\<your-username>\graalpy-venv\Scripts\python.exe -m pip --no-cache-dir install <package>`.

=== Step 4: Configure JSol'Ex

In JSol'Ex, go to **File -> Advanced Parameters -> Python Scripting** and browse to the GraalPy executable inside your virtual environment:

- **Windows**: `C:\Users\<your-username>\graalpy-venv\Scripts\graalpy.exe`
- **Linux / macOS**: `~/graalpy-venv/bin/graalpy`

Restart JSol'Ex for the change to take effect.

=== Troubleshooting

**`pip` is not recognized (Windows)** -- Do not call `pip` or `pip.exe` directly. Use `Scripts\python.exe -m pip --no-cache-dir install ...` from inside the virtual environment.

**NumPy installation fails on Windows** -- GraalPy on Windows does not support native extensions. NumPy cannot be installed. Use Linux or macOS if you need NumPy.

**`GraalPy needs the 'patch' utility` (Linux / macOS)** -- Install the `patch` command using your system package manager (see prerequisites above).

**SSL certificate warnings (Windows)** -- The warning `unable to enumerate Windows certificate store` can be safely ignored; it does not prevent package installation.
...
|`# meta:requires_images = "..."`|Comma separated image kinds the script needs
|`# meta:overrides:<parameter> = "..."`|Forces a processing parameter, whatever the user selected
|===
...
As in ImageMath scripts, an output whose name starts with `__` is not considered an output: it is neither displayed nor saved.
