...
WARNING: **Windows ne supporte pas NumPy ni les autres extensions natives.** GraalPy sous Windows ne peut installer que des packages Python purs (pas d'extensions C, pas de compilation depuis les sources). NumPy, PyArrow et les bibliothèques similaires nécessitant du code natif ne fonctionneront pas. Si vous avez besoin de ces bibliothèques, utilisez Linux ou macOS.
...
Téléchargez et installez GraalPy depuis la https://www.graalvm.org/python/python-developers/[page officielle de GraalPy].
...
NOTE: L'installation peut prendre plusieurs minutes si NumPy doit être compilé depuis les sources.
...
TIP: Sous Windows, l'environnement virtuel place les exécutables dans le dossier `Scripts` (et non `bin`). Pour installer des packages Python purs, utilisez le `python.exe` de l'environnement virtuel avec le flag `--no-cache-dir` : `C:\Users\<votre-utilisateur>\graalpy-venv\Scripts\python.exe -m pip --no-cache-dir install <package>`.
...
|`# meta:requires_images = "..."`|Types d'images requis par le script, séparés par des virgules
|`# meta:overrides:<paramètre> = "..."`|Force un paramètre de traitement, quels que soient les choix de l'utilisateur
|===
...
Comme dans les scripts ImageMath, une sortie dont le nom commence par `__` n'est pas considérée comme une sortie : elle n'est ni affichée ni sauvegardée.
