...
JSol'Ex est un traitement d'images solaires produites par l'intermédiaire du http://www.astrosurf.com/solex/[Sol'Ex de Christian Buil] ou d'autres spectrohéliographes comme le SHG 700 ou vos propres constructions.
Il permet de traiter les fichiers SER capturés avec cet instrument pour produire des images du disque solaire. Il a été inspiré par http://valerie.desnoux.free.fr/inti/[INTI de Valérie Desnoux] mais dispose de nombreuses fonctionnalités uniques.
...
- la longueur d'onde d'observation: s'il n'est pas obligatoire de remplir ce champ, le faire vous permettra de bénéficier de la coloration automatique et de fonctionnalités avancées telles que les calculs de vitesse. En cliquant sur les "..." vous disposez de la possibilité d'ajouter vos propres raies si elles ne sont pas listées. Le champ situé à côté de la liste vous permet de saisir directement une longueur d'onde en angströms, pour une raie que vous ne souhaitez pas conserver dans la liste : laissez-le vide pour utiliser la raie choisie dans la liste. Il s'agit ici de la longueur d'onde de la raie la plus sombre de votre fenêtre de capture.
- Détection de la raie : la manière dont la raie observée est déterminée.
« Sélection manuelle » utilise la longueur d'onde sélectionnée ci-dessus.
« Recherche automatique sur les raies connues » la cherche parmi les raies listées.
« Recherche automatique sur tout le spectre », qui est le mode par défaut, l'identifie parmi toutes les raies profondes du spectre solaire, ce qui permet de traiter une raie qui n'est pas listée du tout.
Lorsque la recherche reconnaît une raie que vous possédez déjà, c'est celle-ci qui est utilisée, afin que sa coloration et ses scripts s'appliquent toujours ; sinon la raie est créée pour ce scan uniquement, nommée d'après le catalogue de raies spectrales lorsqu'elle y figure.
La recherche ne répond que lorsque l'identification est certaine : une fenêtre de capture étroite peut ne pas contenir assez de spectre pour identifier une raie, auquel cas le logiciel se rabat sur les raies listées.
...
- Méthode d'amélioration du contraste : vous pouvez choisir entre Autostretch (JSol'Ex), CLAHE et CLAHE (multi-échelle). Selon votre choix, des paramètres supplémentaires seront disponibles.
...
La méthode CLAHE (multi-échelle) exécute plusieurs passes CLAHE à des tailles de tuiles déduites automatiquement du diamètre du disque solaire et en moyenne les résultats. Cela atténue fortement l'« anneau » brillant que la CLAHE classique peut produire au limbe, tout en rehaussant le contraste local à plusieurs échelles. Seul le facteur de clipping est configurable ; les tailles de tuiles et le nombre de niveaux sont choisis automatiquement.

Viennent ensuite les paramètres de correction des transversalliums, qui permettent de corriger les bandes transversales qui peuvent apparaître sur les images par exemple à cause de poussières sur la fente.
Deux méthodes sont disponibles : la correction de bandes, utilisée par défaut, et la suppression des stries (destripe), plus efficace sur les stries marquées ou irrégulières.
Les deux méthodes n'utilisent que les pixels du disque solaire.

La correction de bandes a les paramètres suivants :
...
La suppression des stries a les paramètres suivants :

- Passes automatiques : lorsque cette case est cochée, les passes sont répétées jusqu'à ce que les stries soient supprimées.
- Passes : le nombre de passes à effectuer lorsque les passes automatiques ne sont pas cochées.
- Largeur : la largeur, en pixels, des stries les plus larges à supprimer. Les structures plus larges sont conservées.

Lorsque les images de débogage sont activées, un graphique est également généré, montrant la correction appliquée à chaque position le long de la fente par chaque passe, ainsi que la correction totale.
...
==== Correction des oscillations de la monture

Ceci est une fonctionnalité expérimentale disponible depuis JSol'Ex 5.3.2.
Certaines montures présentent une oscillation régulière et périodique pendant le scan, qui se traduit par des ondulations sinusoïdales du limbe solaire.
La correction des bords dentelés peut réduire ce défaut, mais elle corrige chaque ligne indépendamment, ce qui la rend sensible au bruit et peut introduire des artéfacts.

La correction des oscillations de la monture effectue à la place une analyse spectrale de la position du limbe pour trouver les fréquences dominantes de l'oscillation, puis ajuste un modèle du déplacement qui est appliqué à l'image entière.
Le modèle est une sinusoïde lorsque l'oscillation est stable, ou une courbe de déplacement lissée lorsque sa période ou son amplitude dérive pendant le scan.
Comme la correction est pilotée par un modèle global plutôt que par des mesures ligne à ligne, elle est insensible au bruit de détection, et elle peut aussi corriger les lignes où le limbe ne peut pas être mesuré de manière fiable, par exemple près des pôles ou en dehors du disque.

Si aucune oscillation périodique significative n'est détectée, aucune correction n'est appliquée.
L'amplitude et la période détectées sont écrites dans le journal, ce qui peut aider à diagnostiquer la monture.
Lorsque les images de débogage sont activées, un graphique est également généré, montrant le décalage mesuré du limbe, le modèle d'oscillation ajusté et le décalage résiduel après correction.
Les deux corrections peuvent être activées ensemble : dans ce cas, l'oscillation est supprimée en premier, et la correction des bords dentelés ne traite que les défauts aléatoires restants.
...
Sur l'image moyenne, la raie spectrale détectée est tracée en rouge.
Lorsque vous sélectionnez une longueur d'onde précise, JSol'Ex vérifie que la raie détectée est bien celle que vous avez demandée : une raie voisine est parfois plus sombre, en particulier sur les fenêtres de découpe étroites choisies autour d'une raie de référence peu marquée.
Lorsqu'il doit se corriger, la raie réellement utilisée pour la reconstruction est tracée en vert, et la rouge montre ce qui avait été détecté.
...
Lorsque le disque solaire est surexposé (saturé), la raie spectrale ne peut plus être distinguée de façon fiable, et JSol'Ex ne peut donc pas détecter le polynôme de distorsion à partir du scan lui-même.
Dans ce cas, vous pouvez capturer un ou plusieurs scans de référence *non saturés* et laisser JSol'Ex réutiliser leur polynôme.

Cochez la case "Mode disque saturé" (sous "Forcer le polynôme") et sélectionnez le répertoire contenant vos scans de référence non saturés.
Lors du traitement d'un fichier, JSol'Ex recherche le scan de référence le plus proche dans le temps (d'après les dates des fichiers SER) et applique son polynôme.

Ce mode est particulièrement utile en traitement par lots : plusieurs scans saturés peuvent partager le même scan de référence, auquel cas le polynôme de référence n'est calculé qu'une seule fois puis réutilisé.

Si le scan de référence le plus proche est éloigné dans le temps, un avertissement est émis car le polynôme peut ne plus être précis.
Si aucun scan de référence utilisable n'est trouvé, JSol'Ex revient à la détection automatique habituelle.

Ce mode modifie également la détection du bord du disque solaire : le halo qui entoure un disque saturé est suffisamment lumineux pour être confondu avec le disque, JSol'Ex recherche donc le bord de la zone saturée.
Le disque détecté est par conséquent légèrement plus petit que le limbe réel, ce qui est inévitable avec un disque saturé.

NOTE: "Mode disque saturé" et "Forcer le polynôme" sont mutuellement exclusifs.
...
Vous pouvez aussi dupliquer une image depuis la barre latérale en faisant un clic droit sur son nom et en choisissant « Dupliquer », ce qui est utile lorsque vous voulez conserver une version originale à côté d'une version annotée.

Les animations et vidéos disposent d'un bouton « Rogner / pivoter... » qui ouvre une fenêtre affichant une seule image (l'image en cours pour une vidéo).
Vous pouvez la pivoter, la retourner ou la rogner, puis cliquer sur « Appliquer » : les opérations sont appliquées à toutes les images et le résultat apparaît comme une nouvelle animation à côté de l'originale.

=== Autres raies spectrales dans la fenêtre de capture

La fenêtre capturée contient souvent des raies spectrales connues autres que celle que vous étudiez. Par exemple, lors d'une capture en Calcium H, la raie H-epsilon se trouve généralement dans la même fenêtre.

Dans ce cas, JSol'Ex signale chacune de ces raies par un marqueur vertical en pointillés, accompagné du nom de la raie, sur le graphe du profil spectral, et reconstruit automatiquement une image dédiée à la position de cette raie, exactement comme si vous aviez ajouté son décalage de pixels manuellement.

=== Annotations

Chaque image a son propre jeu d'annotations que vous pouvez activer ou désactiver indépendamment depuis le panneau d'annotations, ouvert via le bouton ⊕ de la barre d'outils.
Ce panneau est une fenêtre flottante que vous pouvez déplacer à votre guise pendant votre travail.

Les annotations suivantes sont disponibles :

- *Coloriser* : applique une couleur à une image monochrome, utile comme arrière-plan pour des annotations colorées. Cette option n'apparaît que pour les images monochromes. Depuis son menu d'options, vous pouvez choisir un profil spectral (par exemple H-alpha, Calcium, Hélium) ou une couleur personnalisée ; le profil par défaut correspond à la raie spectrale de l'image. Une option « Préserver la luminosité » conserve la luminosité d'origine afin que la colorisation n'ajoute que la teinte sans assombrir l'image. Les images RVB ne sont pas modifiées.
- *Grille d'orientation* : méridiens et parallèles héliographiques tracés sur le disque solaire, avec l'axe de rotation et les marqueurs des directions cardinales. Le style (coordonnées équatoriales ou solaires) et la correction de l'angle P se règlent depuis le menu d'options de la grille.
- *Échelle des protubérances* : cercles concentriques pointillés à l'extérieur du limbe, avec des étiquettes de distance en kilomètres. Vous pouvez choisir le nombre de cercles, l'espacement entre eux, ainsi que l'angle auquel les étiquettes sont dessinées, afin qu'elles ne se superposent pas aux protubérances.
- *Régions actives* : les numéros des régions actives NOAA placés à leur position sur le disque solaire. Depuis le menu d'options, vous pouvez tracer un rectangle autour de chaque région et régler la correction de l'angle P pour les images ImageMath.
- *Détails d'observation* : un bloc de texte généralement placé en haut à gauche avec le nom de l'observateur, la date, l'instrument, le télescope, la caméra, etc. Le contenu est entièrement modifiable via un modèle avec des variables (par exemple `%OBSERVER%`, `%DATETIME%`, `%TELESCOPE%`) : vous choisissez les champs affichés, vous les réorganisez, vous y ajoutez votre propre texte. Les lignes vides (lorsque la valeur est absente) sont automatiquement ignorées.
- *Paramètres solaires* : un bloc listant les angles P, B0, L0 et la rotation de Carrington, placé en haut à droite.
- *Référence de taille (Terre)* : une petite image de la Terre superposée à l'image pour servir de comparaison de taille. Le marqueur est déplaçable à la souris.
- *Signature* : texte libre placé en bas à gauche de l'image. Vous pouvez choisir la police (la liste déroulante affiche chaque famille dans sa propre typographie), la taille et le style.
- *Zones de texte* : un nombre quelconque d'annotations de texte libre que vous placez où vous voulez sur l'image en les faisant glisser. Utilisez le bouton « Ajouter une zone de texte » en bas du panneau pour en créer une ; chacune a son propre texte, sa police, sa taille, son style et sa couleur, et peut être supprimée avec son bouton corbeille.

Chaque annotation a sa propre couleur, gris neutre par défaut afin de préserver les images monochromes.
Un réglage global d'épaisseur de trait en bas du panneau permet d'ajuster l'intensité de tous les traits dessinés.

Lors de la modification de la signature ou du modèle des détails d'observation, les changements sont prévisualisés en direct sur l'image.
Cliquez sur « Appliquer » pour les conserver ou « Fermer » pour les annuler.

Vous pouvez enregistrer les réglages d'annotations actuels (activations, couleurs, épaisseur, options de l'échelle des protubérances, modèle des détails, signature) en tant que préréglage via le bouton « Enregistrer le préréglage » en bas du panneau, et les recharger sur n'importe quelle autre image avec « Charger le préréglage ».
Le préréglage est stocké sur votre ordinateur et conservé entre les sessions de JSol'Ex.

== Exporter et importer une session

Après avoir traité un ou plusieurs fichiers, on souhaite souvent comparer les résultats puis partager la meilleure image, par exemple sur SpectroSolHub.
Plutôt que de retraiter la vidéo SER d'origine, vous pouvez exporter l'état actuel de la visionneuse dans un *fichier de session* et le rouvrir à tout moment.

Pour exporter, choisissez « Exporter la session... » dans le menu *Fichier* et indiquez la destination.
Le fichier de session (`.jsxs`) enregistre toutes les images générées, les animations et vidéos, ainsi que leurs paramètres de traitement.
Le fichier est compressé, mais gardez à l'esprit qu'une session contenant de nombreuses images peut rester volumineuse.

Pour restaurer une session, choisissez « Importer une session... » dans le menu *Fichier* et sélectionnez un fichier `.jsxs`.
Si la visionneuse contient déjà des images, JSol'Ex vous demande si vous souhaitez les remplacer ou fusionner les images importées avec la vue actuelle.
Les images restaurées conservent leurs paramètres d'origine : vous pouvez les enregistrer, les annoter ou les partager exactement comme si elles venaient d'être traitées, sans toucher à la vidéo d'origine.
...
== Importer des scans depuis un SunScan

Si vous possédez un spectrohéliographe SunScan, JSol'Ex peut récupérer directement les scans acquis par l'appareil, sans avoir à copier les fichiers manuellement.

Connectez votre ordinateur au point d'accès Wi-Fi du SunScan, puis dans le menu "Fichier" choisissez "Importer depuis SunScan...".

JSol'Ex tente de localiser automatiquement l'appareil sur le réseau. Si la détection échoue, saisissez son adresse (par exemple `sunscan.local:8000`) dans le champ "Adresse de l'appareil" puis cliquez sur "Connexion".

La fenêtre liste tous les scans disponibles sur l'appareil, avec leur date d'acquisition, leur raie spectrale et leur état de traitement. Cochez les scans à traiter, choisissez un dossier de téléchargement, puis cliquez sur "Importer et traiter". JSol'Ex télécharge les fichiers `scan.ser` sélectionnés (renommés `YYYY_MM_DD-HH_mm_ss-scan.ser` à partir de la date UTC de capture) et les ouvre : un scan unique ouvre la fenêtre de traitement habituelle, tandis que plusieurs scans sont traités en lot.

Lorsque les scans sont importés de cette manière, l'instrument "Sunscan" et son paramétrage (caméra et taille des pixels) sont sélectionnés automatiquement dans les paramètres de traitement, ce qui active également le retournement vertical du spectre. Vous pouvez toujours modifier ces réglages avant de lancer le traitement.
...
[[internal-outputs]]
==== Masquer un résultat intermédiaire

Il arrive qu'un résultat intermédiaire ne serve qu'à la section `[outputs]` elle-même, et que le déplacer dans une autre section rende le script moins lisible.
Dans ce cas, préfixez son nom par `__` : une telle variable est calculée et peut être utilisée par les autres expressions, mais elle n'est pas considérée comme une sortie, elle n'est donc ni affichée ni sauvegardée.

[source]
----
[outputs]
# pas une sortie : uniquement utilisée ci-dessous
__stretched = asinh_stretch(cropped;1000;10)
helium_mono = sharpen(__stretched)
----
...
* **requirements** : Conteneur pour déclarer les types d'images que le script a besoin pour fonctionner (voir <<#script-requirements,Prérequis de script>>)
* **overrides** : Conteneur pour les paramètres de traitement que le script force, quels que soient les choix de l'utilisateur (voir <<#script-overrides,Forcer des paramètres de traitement>>)

[[script-requirements]]
==== Prérequis de script

Un script peut déclarer les types d'images dont il a besoin pour accomplir sa tâche. Lorsque le script est exécuté, toute valeur de `GeneratedImageKind` listée dans le bloc `requirements` est ajoutée à la liste des images à générer, comme si l'utilisateur les avait sélectionnées explicitement dans le panneau de sélection d'images.

[source]
----
meta {
    title = "Régions actives sur le continuum"
    requirements {
...
Les types d'images inconnus sont ignorés silencieusement. Les types induits sont automatiquement inclus (par exemple, `ELLERMAN_BOMBS` implique `FLARES`).

[[script-overrides]]
==== Forcer des paramètres de traitement

Certains scripts n'ont de sens qu'avec des paramètres de traitement particuliers : par exemple, un script qui effectue sa propre suppression des bandes a besoin que la correction des bandes intégrée soit désactivée, et un script qui travaille sur la couronne a besoin d'un recadrage suffisamment large.
Un tel script peut déclarer un bloc `overrides`, qui force des paramètres de traitement quels que soient les choix de l'utilisateur :
...
Les noms des paramètres sont ceux du fichier `config.json` que JSol'Ex enregistre à côté des images générées : n'importe quel paramètre de traitement peut donc être forcé, et il suffit d'ouvrir ce fichier pour trouver le nom du paramètre recherché.
Les plus utiles sont :

[%header,cols="35%,65%"]
|===
|Paramètre|Description
|`bandingCorrectionParams.passes`|Nombre de passes de correction des bandes. Utilisez `0` pour désactiver la correction.
|`bandingCorrectionParams.width`|Taille des bandes, en pixels, pour la correction des bandes.
|`bandingCorrectionParams.method`|Méthode de correction des bandes : `BANDING_CORRECTION` ou `DESTRIPE`.
|`bandingCorrectionParams.destripeParams.passes`|Nombre de passes de suppression des stries, `-1` pour s'arrêter automatiquement.
|`bandingCorrectionParams.destripeParams.bandSize`|Largeur, en pixels, des stries les plus larges supprimées par la suppression des stries.
|`geometryParams.autocropMode`|Recadrage : `OFF`, `RADIUS_1_1`, `RADIUS_1_2`, `RADIUS_1_5`, `SOURCE_WIDTH` ou `FIXED_WIDTH`.
|`geometryParams.fixedWidth`|Largeur utilisée lorsque `autocropMode` vaut `FIXED_WIDTH`.
|===

Les nombres s'écrivent tels quels, tout le reste entre guillemets, y compris les booléens : `spectrumVFlip = "true"`.
Seules des valeurs simples peuvent être forcées, pas des listes.
Les paramètres inconnus et les valeurs invalides sont ignorés, avec un avertissement dans les journaux.
Lorsque plusieurs scripts forcent le même paramètre avec des valeurs différentes, c'est le premier qui l'emporte.

Les paramètres sont forcés avant le traitement de la vidéo, et le fichier `config.json` enregistré à côté des images reflète les valeurs réellement utilisées.

Ces paramètres peuvent aussi être modifiés depuis l'éditeur de métadonnées, et sont supportés par les scripts Python, avec un commentaire par paramètre :
...
Pour des capacités de script avancées, JSol'Ex supporte également Python. Veuillez consulter link:python-scripting.html[la documentation sur le scripting Python] pour plus de détails.
...
<video width="100%" controls autoplay loop>
  <source src="../shared/orig.webm" type="video/webm">
...
++++
<video width="100%" controls autoplay loop>
  <source src="../shared/trimmed.webm" type="video/webm">
  Your browser does not support the video tag.
</video>
++++
...
JSol'Ex supporte les dépôts de scripts, vous permettant de découvrir et télécharger automatiquement des scripts ImageMath publiés par la communauté.
Les scripts de dépôt apparaissent aux côtés des scripts intégrés dans le panneau de sélection d'images et sont automatiquement mis à jour lorsque de nouvelles versions sont publiées.

=== Utilisation des dépôts de scripts

==== Ajouter un dépôt

Pour ajouter un dépôt de scripts :

1. Ouvrez le menu Outils et sélectionnez "Dépôts de scripts..."
2. Cliquez sur le bouton "Ajouter"
3. Entrez un nom pour le dépôt (à des fins d'affichage)
4. Entrez l'URL du dépôt (doit commencer par `http://` ou `https://`)
5. Cliquez sur "Ajouter"

JSol'Ex récupérera immédiatement les scripts disponibles depuis le dépôt.

==== Gestion des dépôts

La boîte de dialogue des dépôts de scripts affiche :

- **Nom** : Le nom du dépôt que vous avez fourni
- **URL** : L'adresse du dépôt
- **Scripts** : Nombre de scripts disponibles
- **Dernière vérification** : Quand le dépôt a été vérifié pour la dernière fois

Vous pouvez :

- **Modifier** : Changer l'URL du dépôt (le nom ne peut pas être modifié)
- **Supprimer** : Supprimer un dépôt et ses scripts mis en cache
- **Actualiser** : Vérifier manuellement les nouveaux scripts ou les mises à jour

==== Utilisation des scripts de dépôt

Les scripts de dépôt apparaissent dans le panneau de sélection d'images aux côtés des scripts intégrés.
Ils sont organisés par nom de dépôt et affichent les métadonnées incluant :

- Titre du script
- Nom de l'auteur
- Numéro de version

Sélectionnez les scripts de dépôt de la même manière que les scripts intégrés.
Si un script déclare des paramètres dans son bloc de métadonnées, vous pourrez les configurer dans l'interface.

==== Mises à jour automatiques

JSol'Ex vérifie les dépôts pour les mises à jour :

- À chaque démarrage
- Lorsque vous cliquez sur le bouton Actualiser dans la boîte de dialogue des dépôts de scripts

Les scripts sont automatiquement mis à jour lorsqu'une version plus récente est détectée en fonction du champ version dans les métadonnées du script.

**Compatibilité de version**

Lors de la vérification des mises à jour, JSol'Ex assure la compatibilité de version :

- Les scripts sans champ `requires` sont toujours téléchargés (considérés comme compatibles avec toutes les versions)
- Les scripts avec `requires` défini à une version plus récente que votre version actuelle de JSol'Ex sont ignorés
- Si vous avez une version locale compatible d'un script, elle est préservée même si une version plus récente incompatible est publiée

=== Publication de scripts

Si vous souhaitez partager vos scripts ImageMath avec la communauté, vous pouvez les publier via un dépôt de scripts.

==== Structure du dépôt

Un dépôt de scripts est simplement un serveur HTTP qui expose des fichiers `.math`.
JSol'Ex supporte deux méthodes de découverte :

**1. Fichier scripts.txt (recommandé)**

Créez un fichier `scripts.txt` listant les scripts disponibles (un nom de fichier par ligne) :
...
**2. Liste de répertoires (solution de repli)**

Activez la liste de répertoires sur votre serveur web (Apache `mod_autoindex`, nginx `autoindex`) :
...
JSol'Ex essaiera d'abord de télécharger `scripts.txt`, et se rabattra sur l'analyse des listes de répertoires HTML si le fichier n'est pas trouvé.

==== Support des fichiers Zip

Pour les scripts nécessitant des fichiers supplémentaires (comme des modules Python), vous pouvez les regrouper dans une archive `.zip`.
JSol'Ex extraira automatiquement le contenu lors du téléchargement depuis un dépôt.
...
----
mon-script.zip
├── mon-script.math      # Script principal
├── main.txt             # Optionnel : spécifie le script principal (requis si plusieurs fichiers .math)
├── utils.py             # Modules Python
└── helpers.py
----

**Règles pour déterminer le script principal :**

1. S'il n'y a qu'un seul fichier `.math` dans le zip, il est automatiquement utilisé comme script principal
2. S'il y a plusieurs fichiers `.math`, vous devez inclure un fichier `main.txt` contenant le nom du script principal

La structure des répertoires à l'intérieur du zip est préservée, permettant aux scripts de référencer des fichiers avec des chemins relatifs comme `python_file("utils.py")`.
...
Listez les fichiers zip dans `scripts.txt` aux côtés des fichiers `.math` :

[source]
----
# Mon dépôt
script-simple.math
script-avec-modules.zip
----

==== Exigences pour les scripts

Pour qu'un script soit disponible dans un dépôt, il doit :

1. Avoir une extension `.math`
2. Inclure un bloc de métadonnées avec les champs requis
3. Être accessible via HTTP/HTTPS

==== Format du bloc de métadonnées

Chaque script doit inclure un bloc de métadonnées avec au minimum :

- `author` : Nom de l'auteur du script
- `title` : Titre du script (affiché dans l'interface, peut être multilingue)
- `version` : Version du script pour le suivi des mises à jour

Les champs optionnels incluent :

- `requires` : Version minimale de JSol'Ex requise (par exemple, `"4.2.0"`). Les utilisateurs exécutant des versions plus anciennes ne téléchargeront pas les scripts avec des exigences de version incompatibles.
- `description` : Description du script (peut être multilingue)
- `params` : Déclarations de paramètres avec types, valeurs par défaut et descriptions

Exemple avec support multilingue et paramètres :
...
Le bloc de métadonnées doit apparaître au début du fichier script.

==== Mises à jour de version

Lorsque vous publiez une nouvelle version d'un script, mettez à jour le champ `version` dans le bloc de métadonnées.
Il est recommandé d'utiliser le versionnement sémantique (par exemple, `1.0`, `1.1`, `2.0`).

Les utilisateurs recevront automatiquement le script mis à jour la prochaine fois que JSol'Ex vérifiera les mises à jour.

==== Exemple de dépôt

Un dépôt de scripts minimal pourrait ressembler à :
...
Et `empilement.math` avec métadonnées complètes et paramètres :

++++
<details>
<summary><strong>Cliquez pour développer l'exemple complet du script</strong></summary>
++++
...
[TIP]
====
Envisagez d'héberger vos scripts sur GitHub Pages, GitLab Pages, ou tout service d'hébergement statique permettant la liste de répertoires ou le service de fichiers texte.
====
...
WARNING: Le balayage aller-retour est fortement déconseillé. La plupart des montures ont un sens de balayage préférentiel, pour lequel la vitesse de balayage est plus régulière : balayer dans les deux sens risque donc de distordre la moitié de vos images. N'utilisez cette option que si vous êtes absolument certain que les scans dans les deux sens sont parfaits.

Certains observateurs balayent alternativement dans un sens, puis dans l'autre, afin d'éviter d'avoir à ramener le télescope à son point de départ entre deux scans.
Comme un scan effectué dans le sens inverse produit une image en miroir, il faudrait retourner une image sur deux à la main.

Cochez l'option « Balayage aller-retour », dans la section « Options de traitement par lot » des paramètres de traitement, pour que JSol'Ex le fasse à votre place.
Les fichiers du lot sont alors triés par heure de capture, et un scan sur deux, à partir du second, est retourné automatiquement.

NOTE: Si le retournement s'applique aux mauvais scans, par exemple parce que votre premier fichier est un scan dans le sens inverse, cochez simplement l'option « Inverser l'axe horizontal » en plus : cela inverse les scans qui sont retournés.

[[inversion-conditionnelle]]
=== Inversions conditionnelles

Lorsque l'orientation des images change en cours de session, par exemple après un retournement au méridien, une simple case « inverser » ne peut pas décrire un lot où certains fichiers doivent être retournés et d'autres non.

Les options « Inversion horizontale conditionnelle » et « Inversion verticale conditionnelle », dans la section « Options de traitement par lot » des paramètres de traitement, résolvent ce problème.
Lorsque vous activez l'une d'elles, vous définissez une date pivot, en UTC, et choisissez si ce sont les fichiers capturés avant la date pivot, ou ceux capturés à cette date ou après, qui doivent être retournés.
Les autres fichiers ne sont pas retournés.

Plutôt que de saisir la date pivot à la main, vous pouvez cliquer sur « Depuis un fichier SER... » et sélectionner la première vidéo capturée après le changement d'orientation : sa date de capture est alors utilisée comme date pivot.

NOTE: Lorsqu'une inversion conditionnelle est activée, elle remplace la case « Inverser l'axe horizontal » ou « Inverser l'axe vertical » correspondante, qui est ignorée pour cet axe.

Lorsque le mode disque saturé est également activé, seuls les scans de référence capturés du même côté de la date pivot que le fichier en cours de traitement sont pris en compte : un retournement au méridien modifie les flexions mécaniques, et un scan de référence de l'autre côté donnerait un polynôme biaisé.
Dans ce cas, veillez à ce que la date pivot corresponde à la première capture après le retournement, scans de référence compris : un scan de référence capturé entre le retournement et le fichier choisi comme pivot serait sinon considéré à tort comme capturé avant le retournement.

[[ajout-fichiers-lot]]
=== Ajouter des fichiers à un lot

Une fois qu'un lot est terminé, vous pouvez y ajouter des fichiers sans avoir à retraiter ceux qui sont déjà traités.
Les nouveaux fichiers sont traités avec les mêmes paramètres que le reste du lot, puis, si votre script comporte une section `\[[batch]]`, les résultats du lot sont recalculés sur l'ensemble des fichiers, anciens et nouveaux.
Ils apparaissent dans leur propre section horodatée, ce qui permet de les comparer avec les précédents.

Pour cela, cliquez sur le bouton "Ajouter des fichiers...", en haut du tableau du lot, et sélectionnez les fichiers à ajouter.

Vous pouvez également cliquer sur le bouton "Surveiller un dossier..." et choisir un dossier.
JSol'Ex surveille alors ce dossier : chaque nouveau fichier SER qui y apparaît est automatiquement ajouté au lot.
C'est particulièrement pratique pendant une session d'observation : lancez un lot avec les premiers scans, puis laissez JSol'Ex ajouter les suivants au fur et à mesure que votre logiciel d'acquisition les enregistre.

Les fichiers ne sont ajoutés au lot que lorsque le traitement en cours est terminé : les fichiers détectés pendant que le lot tourne sont mis en attente et seront ajoutés dès que celui-ci sera fini.
Seuls les fichiers qui apparaissent après le début de la surveillance sont pris en compte : ceux qui sont déjà présents dans le dossier sont ignorés.
Dans tous les cas, que vous ajoutiez les fichiers à la main ou via le dossier surveillé, un fichier déjà présent dans le lot n'est jamais ajouté une seconde fois.
Cliquez sur "Arrêter la surveillance" pour cesser de surveiller le dossier.
...
L'identification utilise la même méthode que l'identification en direct du serveur embarqué (voir <<live-line-identification>>) : la raie au centre de l'image est recherchée parmi toutes les raies profondes du spectre solaire, tel qu'observé à travers l'atmosphère, et toutes les autres raies visibles dans l'image sont également identifiées, y compris les raies telluriques.
L'orientation de l'image est détectée automatiquement.
...
Les raies trouvées dans l'image sont marquées en vert, et la barre d'état indique sur quelle raie l'image est centrée et le niveau de confiance de l'identification.

La case "Raies telluriques" affiche le spectre de référence tel qu'observé à travers l'atmosphère, c'est-à-dire ce que votre caméra capture : décochez-la pour voir le spectre solaire seul.
...
=== Identification en direct des raies spectrales

Le serveur embarqué expose également une API qui identifie les raies spectrales visibles dans une image du spectre, par exemple la vue en direct de votre logiciel de capture.
Vous savez ainsi quelle raie vous observez pendant que vous réglez le réseau de votre spectrohéliographe, sans interrompre la capture.

Un script pour SharpCap est fourni : choisissez "Scripts SharpCap" dans le menu "Aide" pour ouvrir le dossier où JSol'Ex le copie (il est également disponible https://github.com/melix/astro4j/blob/main/scripts/sharpcap/jsolex_spectral_lines.py[sur GitHub]).
Une fois JSol'Ex lancé avec son serveur embarqué démarré, exécutez le script depuis la console de scripts de SharpCap.
Il envoie les images en direct à JSol'Ex et dessine chaque raie identifiée par dessus la vue en direct, avec son nom lorsqu'il s'agit d'une raie connue ou sa longueur d'onde sinon.
La raie au centre de la fenêtre est mise en évidence, et une ligne d'état indique la confiance de l'identification, qui augmente à mesure que les images successives s'accordent sur la même raie.
Une raie est proposée dès que l'image contient un spectre : lorsque la fenêtre est trop étroite pour qu'une raie se distingue de toutes les autres, la plus probable est tout de même affichée, mais la confiance signale qu'il s'agit d'une supposition et sa raie est tracée en pointillés.
Élargissez la fenêtre de capture ou moyennez davantage d'images avant de vous fier à une telle réponse.
Arrêtez le script avec Ctrl+C pour retirer l'affichage.

Une petite fenêtre de réglages s'ouvre avec le script.
Elle permet de limiter le nombre de raies étiquetées, qui sont alors réparties uniformément sur la fenêtre tandis que les autres ne sont marquées que par un trait, et de choisir la taille des étiquettes, qui suit le zoom de l'affichage.
Elle propose aussi un mode "Rechercher une raie" : saisissez la longueur d'onde recherchée, en Angströms, et le script ne dessine que cette raie lorsqu'elle est dans la fenêtre, accompagnée d'une règle graduée en Angströms à droite du spectre ; lorsque la raie est hors de la fenêtre, la ligne d'état indique de combien elle dépasse et de quel côté, afin que vous sachiez dans quel sens tourner le réseau.

Le binning est lu depuis la caméra, et l'orientation du spectre est détectée d'après la courbure des raies, de sorte que le sens dans lequel le logiciel de capture lit le capteur n'a pas d'importance.
Le spectrohéliographe et la taille des pixels de la caméra sont ceux des détails d'observation de JSol'Ex, sauf si le script les remplace : modifiez les paramètres en tête du script pour indiquer l'adresse du serveur, la taille des pixels ou le nom du spectrohéliographe.
L'identification est d'autant plus fiable que la fenêtre de capture est haute, puisqu'une plus grande partie du spectre est comparée au spectre de référence, et que davantage d'images sont moyennées.
L'image est comparée à une référence qui inclut les raies d'absorption de l'atmosphère terrestre, tirées de l'atlas NSO/Kitt Peak : autour de H-alpha, les raies de l'eau sont aussi profondes que les raies solaires, et un spectre observé depuis le sol ne peut pas être reconnu sans elles.
Ces raies sont étiquetées comme telluriques.

L'API peut être utilisée depuis tout autre logiciel : envoyez une image par une requête `POST` à `/api/spectrum/identify`.
Le corps est soit les pixels bruts de l'image, avec les paramètres `width`, `height` et `format`, `format` étant l'espace de couleurs de la caméra (`MONO8`, `MONO12`, `MONO16`, `RGB24`, `RGB32`, `RGB48`, `RAW8`, `RAW12` ou `RAW16`, les échantillons sur plusieurs octets étant petit-boutistes), soit un fichier image (PNG, TIFF, ou FITS avec `format=fits`).
Les images couleur sont moyennées en mono, et les images portant une matrice de Bayer sont conservées en pleine résolution, la matrice étant égalisée.
Les paramètres optionnels `pixelSize`, `binning`, `instrument` et `average` remplacent les détails d'observation et le nombre d'images moyennées.
La réponse est un document JSON décrivant le polynôme de courbure des raies, la raie identifiée avec son score, la dispersion, et la liste des raies trouvées dans la fenêtre avec leur position par rapport au centre de l'image.
Une requête `DELETE` à la même adresse oublie les images précédentes, par exemple lorsque vous changez de caméra.
...
- Le National Solar Observatory pour l'atlas du spectre tellurique utilisé par l'identification en direct des raies spectrales : NSO/Kitt Peak FTS data used here were produced by NSF/NOAO
