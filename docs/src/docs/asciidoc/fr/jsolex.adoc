...
- `transition` prend une liste d'images en paramètre, ainsi qu'un nombre d'étapes, et génère une nouvelle liste d'images en interpolant entre les images de la liste. Par exemple: `transition(list(img(0), img(1)), 10)` génère 10 images entre img(0) et img(1). Il est possible d'ajouter un paramètre optionnel pour spécifier le type de transition : `linear`, `ease_in`, `ease_out` ou `ease_in_out`. Par exemple: `transition(list(img(0), img(1)), 10, "ease_in")`.
...
- `wavelen` renvoie la longueur d'onde calculée d'une image, en fonction de la dispersion calculée et de la longueur d'onde de référence. Par exemple : `wavelen(img(0))`.
...
- `remote_scriptgen` est une fonction expérimentale et avancée qui est décrite spécifiquement dans <<#remote-scriptgen, cette section>>.

[[batch-mode]]
[[special-variables]]
=== Variables spéciales

La table ci-dessous résume les variables spéciales exposées aux scripts ImageMath :

[%header,cols="25%,75%"]
|===
|Variable|Description
|`blackPoint`|La valeur du point noir détectée dans l'image
|`angleP`|La valeur de l'angle P (en radians)
|`b0`|La valeur de l'angle B0 (en radians)
|`l0`|La valeur de l'angle L0 (en radians)
|`carrot`|La rotation de Carrington
|`detectedWavelen`|La valeur de la longueur d'onde détectée (en Angström), correspondant à l'image `img(0)`
|===
