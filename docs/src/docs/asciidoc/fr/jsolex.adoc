...
- `crop_sunspots` génère une liste d'images correspondant aux taches solaires détectées. Par exemple: `crop_sunspots(continuum())`. Elle accepte un paramètre optionnel qui est la taille minimale des taches, en pixels. Par exemple: `crop_sunspots(continuum(), 10)`. La taille par défaut est de 32 pixels.
