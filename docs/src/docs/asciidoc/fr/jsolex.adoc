...
Cela donne accès aux structures de contrôle Python (boucles, conditions, gestion des exceptions) tout en permettant d'appeler toutes les fonctions ImageMath via le module `jsolex`.
...
Le code Python peut être spécifié en ligne :
...
Une des fonctionnalités les plus intéressantes est la possibilité d'appeler n'importe quelle fonction ImageMath depuis Python, permettant ainsi une interaction complète bidirectionnelle entre les 2 langages :

[source,python]
----
# Appeler n'importe quelle fonction ImageMath : jsolex.funcs.NOM_FONCTION(args)
img = jsolex.funcs.img(0)                      # Récupérer l'image au décalage 0
...
Toutes les fonctions ImageMath sont disponibles via `jsolex.funcs`, en majuscules ou minuscules :
...
==== Accéder aux variables avec jsolex.vars
...
# Écrire des variables (doit être déclarée d'abord dans ImageMath)
jsolex.vars.my_result = processed_image
----

==== Le module jsolex

Les scripts Python ont accès au module `jsolex`, disponible en global ou via import :

[source,python]
----
# En global (dans les scripts en ligne)
img = jsolex.funcs.img(0)
...
Les scripts Python doivent assigner leur résultat à la variable `result` :
...
Python fournit les structures de contrôle qui manquent à ImageMath :
...
# Conditions
if img.width() > 1500:
    result = jsolex.funcs.rescale_rel(img, 0.5)
else:
    result = img

# Gestion des exceptions
try:
    result = jsolex.funcs.load("/chemin/vers/fichier.fits")
except:
    result = jsolex.funcs.img(0)
----
